...
error_reporting(E_NOTICE);
	...
	@session_start();
	include("../../shoponline2/connectdb.php");
	...
	if(isset($_GET['id'])) {
		$_SESSION['sid'][$id] = $data['j_id'];
		$_SESSION['sname'][$id] = $data['j_name'];
		$_SESSION['sprice'][$id] = $data['j_price'];
		$_SESSION['sdetail'][$id] = $data['j_detail'];
		$_SESSION['spicture'][$id] = $data['j_picture'];
		@$_SESSION['sitem'][$id]++;
	}
?>
<!doctype html>
...
<link href="../../shoponline2/bootstrap.css" rel="stylesheet" type="text/css">
...
<blockquote>
<h2>ตะกร้าสินค้า</h2>
<a href="allproduct.php" class="btn btn-primary">กลับไปเลือกสินค้า</a>  
<a href="../../shoponline2/clear.php" class="btn btn-warning">ลบสินค้าทั้งหมด</a> 
<?php
if(empty($_SESSION['sid'])) {
?>
<a href="#" class="btn btn-success" onClick="alert('กรุณาเลือกสินค้า');">สั่งซื้อสินค้า</a> 
<?php } else { ?>
<a href="../../shoponline2/record.php" class="btn btn-success">สั่งซื้อสินค้า</a>
<?php } ?>
...
?>
	<tr>
		...
		<td><a href="../../shoponline2/clear2.php?id=<?=$pid;?>" class="btn btn-danger">ลบ</a></td>
	</tr>
<?php } // end foreach ?>
	<tr>
